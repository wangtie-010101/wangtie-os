...
function werunFile() {
  return join2(configDir(), "werun.json");
}
function loadWeRunMap() {
  try {
    return JSON.parse(readFileSync2(werunFile(), "utf8"));
  } catch {
    return {};
  }
}
function saveWeRunEntry(date, steps) {
  ...
  map[date] = steps;
  writeFileSync2(werunFile(), JSON.stringify(map, null, 2), "utf8");
}
...
function staticDisposers(webServer, routePrefix) {
  ...
  const serveFile = (response, relative) => {
    ...
    response.end(readFileSync3(target));
  };
